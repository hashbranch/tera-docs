---
title: "GLM-5.1"
description: "Zhipu's open-weight 754B MoE reasoning model on Tera. OpenAI-compatible. 200k context. US-owned GPUs, zero retention."
---

<Info>
  Model id: `zai-org/GLM-5.1` — pass this as the `model` field. The API surface is the OpenAI Chat Completions API; existing OpenAI SDKs and any gateway that abstracts over OpenAI-compatible providers work without code changes.
</Info>
...
| **Quantization** | `fp8` (official `zai-org/GLM-5.1-FP8`), prefix caching enabled |
| **Reasoning** | Emits `reasoning_content` (GLM reasoning parser) |
...
resp = client.chat.completions.create(
    ...
    messages=[{"role": "user", "content": "Explain the tradeoffs of MoE vs dense models in 3 sentences."}],
    max_tokens=512,
)
...
GLM-5.1 is a reasoning model. Its vLLM reasoning parser returns chain-of-thought tokens in a separate `reasoning_content` field so they don't pollute `content`. OpenAI SDKs that expect a plain `content` string continue to work.

<Note>
  OpenAI's spec calls this field `reasoning`. The GLM parser emits it as `reasoning_content`. Treat the two as aliases — if you're porting code that reads `reasoning`, read `reasoning_content` here.
</Note>
...
GLM-5.1 runs with auto tool choice enabled. The request and response shapes match the OpenAI Chat Completions API 1:1.
...
| Stage | Tokens | Cost |
|---|---:|---:|
| User prompt + system | 800 in | $0.0011 |
| Reasoning + tool call | 400 out | $0.0018 |
| Tool result + final answer | 200 in / 300 out | $0.0016 |
| **Per turn** |  | **~$0.0045** |

At 10,000 turns/day this runs ~$45/day. Volume committed-use pricing available — email [hello@tera.gw](mailto:hello@tera.gw).

## Onboard

1. Email [hello@tera.gw](mailto:hello@tera.gw?subject=Tera%20API%20access%20%E2%80%94%20GLM-5.1) — tell us expected concurrency, peak QPS, and rough token volumes.
