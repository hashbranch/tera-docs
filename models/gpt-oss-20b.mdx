...
- **Concurrency**: Current deployment caps `max_num_seqs=12` per replica. Capacity scales horizontally via additional replicas — coordinate with us before driving sustained high concurrency.
...
- **Validation status.** gpt-oss-20b is a validation candidate. Capacity is spun up on demand for the first integration partner; benchmarked TTFT and throughput will be published after first production routing. For comparison, our Qwen3.5-27B on a comparable Tera deployment measures ~280ms TTFT and ~45 tok/s sustained.
