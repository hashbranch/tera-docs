---
title: "gpt-oss-20b"
description: "OpenAI's open-weight 20B reasoning model on Tera. OpenAI-compatible. 131k context. US-only, zero retention."
---
...
| **Hardware** | 1× NVIDIA A100 80GB (SXM4), prefix caching enabled |
| **Reasoning** | Emits `reasoning_content` (OpenAI gpt-oss parser) |
| **Tool calling** | OpenAI-format, auto tool choice enabled |
| **Status** | Private beta — capacity is spun up on demand per integration partner |
...
gpt-oss-20b runs with the OpenAI gpt-oss reasoning parser. Chain-of-thought tokens are returned in a separate `reasoning_content` field so they don't pollute `content`. OpenAI SDKs that expect a plain `content` string continue to work.
...
```json non-streaming
{
  "choices": [{
    "message": {
      "role": "assistant",
      "reasoning_content": "The user is asking about... I should think about...",
...
| `reasoning_content` field | ✅ | Sibling of `content`; absent on non-reasoning models |
...
- **Concurrency**: Current deployment caps `max_num_seqs=12` per A100. Capacity scales horizontally via additional replicas — coordinate with us before driving sustained high concurrency.
...
No preset per-key limit during private beta. Sustained concurrency is coordinated per integration partner; we provision additional replicas before turning up traffic. If you need a specific concurrency or token-per-second guarantee, tell us the shape and we'll size to it.
...
| Stage | Tokens | Cost |
|---|---:|---:|
| User prompt + system | 800 in | $0.0006 |
| Reasoning + tool call | 400 out | $0.0016 |
| Tool result + final answer | 200 in / 300 out | $0.0014 |
| **Per turn** |  | **~$0.0036** |

At 10,000 turns/day this runs ~$36/day. Volume committed-use pricing available — email [hello@tera.gw](mailto:hello@tera.gw).

## Caveats

- **Validation status.** gpt-oss-20b is a validation candidate. Capacity is spun up on demand for the first integration partner; benchmarked TTFT and throughput will be published after first production routing. For comparison, our Qwen3.5-27B on the same A100 hardware measures ~280ms TTFT and ~45 tok/s sustained.
- **Cold start.** First request after VM start: 2–12s TTFT (CUDA graph compile). Warm thereafter.
- **MXFP4 runtime.** Native MXFP4 weights via the experimental vLLM image. We pin the runtime image by digest so model behavior doesn't drift on tag rebuilds.
...
2. We issue an `sk-tera-...` key and bring up capacity in the relevant region.
3. Smoke-test against `https://api.tera.gw/v1` from your gateway.
4. Ramp.

Bring the `X-Tera-Request-ID` of any failing request and we can grep the gateway logs end-to-end.
