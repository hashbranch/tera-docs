---
title: "Kimi K2.6"
description: "Moonshot's open-weight 1T MoE reasoning model on Tera. OpenAI-compatible. 256k context. US-owned GPUs, zero retention."
---

<Info>
  Model id: `moonshotai/Kimi-K2.6` — pass this as the `model` field. The API surface is the OpenAI Chat Completions API; existing OpenAI SDKs and any gateway that abstracts over OpenAI-compatible providers work without code changes.
</Info>
...
| **Quantization** | `int4`, prefix caching enabled |
| **Reasoning** | Emits `reasoning_content` (kimi_k2 parser) |
...
resp = client.chat.completions.create(
    ...
    messages=[{"role": "user", "content": "Draft a 3-step plan to migrate a service to a new database."}],
    max_tokens=512,
)
...
Kimi K2.6 is a reasoning model. Its vLLM reasoning parser (`kimi_k2`) returns chain-of-thought tokens in a separate `reasoning_content` field so they don't pollute `content`. OpenAI SDKs that expect a plain `content` string continue to work.

<Note>
  OpenAI's spec calls this field `reasoning`. The kimi_k2 parser emits it as `reasoning_content`. Treat the two as aliases — if you're porting code that reads `reasoning`, read `reasoning_content` here.
</Note>
...
Kimi K2.6 runs with the `kimi_k2` tool-call parser and auto tool choice enabled. The request and response shapes match the OpenAI Chat Completions API 1:1.
...
| `reasoning_content` field | ✅ | Sibling of `content` for chain-of-thought. OpenAI-spec alias is `reasoning`. |
...
| Image inputs | ❌ | Text-only on Tera today |
...
| Stage | Tokens | Cost |
|---|---:|---:|
| User prompt + system | 800 in | $0.0010 |
| Reasoning + tool call | 400 out | $0.0018 |
| Tool result + final answer | 200 in / 300 out | $0.0016 |
| **Per turn** |  | **~$0.0044** |

At 10,000 turns/day this runs ~$44/day. Volume committed-use pricing available — email [hello@tera.gw](mailto:hello@tera.gw).

## Onboard

1. Email [hello@tera.gw](mailto:hello@tera.gw?subject=Tera%20API%20access%20%E2%80%94%20Kimi-K2.6) — tell us expected concurrency, peak QPS, and rough token volumes.
